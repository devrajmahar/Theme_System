...
export default function Home() {
  ...
  return <div className="min-h-screen bg-background text-foreground">
    ...
    <main className="mx-auto max-w-[1440px] px-4 py-6 sm:px-6 sm:py-10">
      ...
      <div className="mb-6 flex w-fit max-w-full overflow-x-auto rounded-[var(--radius-large)] border bg-surface p-1">{["Components","Fancy Button","States & Icons","Dashboard","Tokens"].map((item) => <button key={item} onClick={() => setTab(item)} className={`whitespace-nowrap rounded-[var(--radius-large)] border border-transparent px-3 py-1.5 text-sm font-medium outline-none transition-all focus-visible:border-ring focus-visible:ring-[2px] focus-visible:ring-ring/50 ${tab === item ? "bg-active-bg text-text-primary" : "text-text-secondary hover:bg-hover-bg hover:text-text-primary active:bg-active-bg"}`}>{item}</button>)}</div>
      <section className="editor-grid section-enter mb-10 rounded-xl border bg-muted/20 p-3 sm:p-5" style={{ animationDelay: "80ms" }}>{tab === "Components" && <ComponentsPreview />}{tab === "Fancy Button" && <FancyButtonShowcase />}{tab === "States & Icons" && <StatesAndIconsPreview />}{tab === "Dashboard" && <DashboardPreview activeNav={activeNav} setActiveNav={setActiveNav} />}{tab === "Tokens" && <TokenPreview />}</section>
      <PalettePlayground />
    </main>
